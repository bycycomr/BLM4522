<#
================================================================================
 BLM4522 - Proje 7: Yedekleme ve Otomasyon - Otomatik Test Runner
--------------------------------------------------------------------------------
 sql/ klasorundeki 5 script'i sirayla calistirir.
 Kullanim: PS> .\run-all.ps1   |   PS> .\run-all.ps1 -Server "MAKINE\INSTANCE"
 NOT: SQL Server servisi calismali. Agent job'larinin ZAMANINDA calismasi icin
      ayrica (yonetici): net start SQLSERVERAGENT
================================================================================
#>
[CmdletBinding()]
param([string]$Server = ".")
...
Write-Host " BLM4522 - Proje 7: Yedekleme Otomasyonu Testleri" -ForegroundColor Yellow
Write-Host " Server : $Server  |  Output : $OutDir"
Write-Host "========================================================" -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Info "Ayrintili hata: $ping"; exit 1
}
Write-Ok "SQL Server erisilebilir. ($($ping -join ' '))"

# Agent durumu bilgisi (zorunlu degil ama uyari verelim)
$agent = (Get-Service -Name SQLSERVERAGENT -ErrorAction SilentlyContinue)
if ($agent -and $agent.Status -ne 'Running') {
    Write-Info "Bilgi: SQL Server Agent '$($agent.Status)'. Job'lar olusturulur ama"
    Write-Info "       zamaninda calismasi icin: net start SQLSERVERAGENT (yonetici)"
}
...
foreach ($f in $scripts) {
    ...
    if (-not (Test-Path $path)) { Write-Fail "Eksik: $f"; $results += [pscustomobject]@{ Script=$f; Status="MISSING" }; continue }
    ...
    if ($LASTEXITCODE -eq 0) { Write-Ok $f; $results += [pscustomobject]@{ Script=$f; Status="OK" } }
    else {
        Write-Fail "$f exit=$LASTEXITCODE (log: $(Split-Path -Leaf $logFile))"
        $results += [pscustomobject]@{ Script=$f; Status="FAIL" }
        if ($f -eq "01_setup_database.sql") { Write-Fail "Setup basarisiz, kalan atlandi."; break }
    }
}
...
"BLM4522 - Proje 7 Test Runner"   | Out-File $summary
